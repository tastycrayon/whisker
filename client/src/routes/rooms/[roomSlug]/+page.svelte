<script lang="ts">
	...
	reader.subscribe((m) => {
		...
		switch (m.type) {
			case EventType.Info:
				if ($userPreference.hideWelcomeMessage) break; // do nothing
			case EventType.Text:
			...
			case EventType.MessageHistory:
				...
				if (Array.isArray(m.payload)) {
					const feed = $userPreference.hideWelcomeMessage
						? m.payload.filter((e) => e.type !== EventType.Info)
						: m.payload; // remove welcome messages
					messageFeed = feed.map((e) => e.payload);
				}
				break;
			default:
