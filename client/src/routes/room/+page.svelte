<script lang="ts">
	...
	let currentMessage = '';
	let timer: number;
	$: roomSlug = $page.url.searchParams.get('slug');
	$: if (!roomSlug) goto(ROOM_PATH);
	$: if (roomSlug && roomSlug !== $currentRoom) currentRoom.set(roomSlug);
	...
	onMount(() => {
		...
		socket.onclose = (event) => {
			...
			timer = setTimeout(async () => {
				await refreshRooms();
				const currentRoom = $roomStore.data.find((e) => e.slug === roomSlug);
	...
	$: messageList = messageFeed.filter((e) => e.roomSlug == roomSlug);
</script>
