...
RUN cd client && npm i && \
    PUBLIC_POCKETBASE_URL=https://whisker.fly.dev \
    PUBLIC_WEBSOCKET_URL=wss://whisker.fly.dev/ws \ 
    npm run build 
RUN ENVIRONMENT=production go build -tags netgo -ldflags '-s -w' -o output main.go
